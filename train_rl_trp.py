"""
train_rl_trp.py — Training + Evaluation Script with Transition Priors
======================================================================

Same as train_rl.py but uses rl_agentwithtrp.py which includes:
  - Transition priors dictionary for cascade prediction
  - 21-dimensional state vector (vs 20 in original)
  - Better correlation chain detection for hard task

Runs:
1. Rule-based baselines (RuleBasedAgent, ImprovedRuleBasedAgent)
2. PPO RL agent training across all 3 tasks
3. Saves all results to results/comparison_results_trp.json

Usage:
    python train_rl_trp.py [--episodes 300] [--eval-episodes 20] [--seed 42]
"""
...
# Import from the transition-prior-enhanced agent instead of rl_agent
from rl_agentwithtrp import PPOTrainer, encode_state, _ACTION_NAMES
...
def run(args):
    # Print info about transition priors
    from rl_agentwithtrp import TRANSITION_PRIORS, USE_TRANSITION_PRIORS, STATE_DIM
    print(f"\n{'='*60}")
    print(f"PPO Training with Transition Priors")
    print(f"{'='*60}")
    print(f"  USE_TRANSITION_PRIORS: {USE_TRANSITION_PRIORS}")
    print(f"  STATE_DIM: {STATE_DIM}")
    print(f"  Transition priors: {len(TRANSITION_PRIORS)} entries")
    ...
    for task_id in ["easy", "medium", "hard"]:
        ...
        print(f"\n[2/3] Training PPO+TRP agent ({n_episodes} episodes)…")
        env     = AdaptiveAlertTriageEnv(task_id=task_id)
        ...
        trainer.save(f"weights/ppo_trp_{task_id}.json")

        # 3. PPO evaluation
        print(f"\n[3/3] Evaluating PPO+TRP agent ({args.eval_episodes} episodes)…")
        ppo_agent = PPOAgentWrapper(trainer)
        ppo_res   = evaluate_agent(ppo_agent, task_id, args.eval_episodes, seed_offset=200)

        print(f"  PPO+TRP      : mean={ppo_res['mean']:.3f}  "
    ...
    out_path = "results/comparison_results_trp.json"
    ...
    # Summary table
    print(f"\n{'='*60}")
    print("FINAL COMPARISON SUMMARY (with Transition Priors)")
    print(f"{'='*60}")

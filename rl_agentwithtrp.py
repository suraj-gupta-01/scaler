...
# ── Transition Priors ─────────────────────────────────────────────────────
# Probability that one alert type cascades into another alert type.
# These priors help the agent anticipate correlated failure chains.
TRANSITION_PRIORS = {
    ("CPU", "MEMORY"): 0.7,
    ("MEMORY", "DISK"): 0.6,
    ("DISK", "NETWORK"): 0.5,
    ("NETWORK", "APPLICATION"): 0.4,
    ("APPLICATION", "SECURITY"): 0.3,
    ("SECURITY", "CPU"): 0.2,
    # Reverse transitions (lower probability)
    ("MEMORY", "CPU"): 0.3,
    ("DISK", "MEMORY"): 0.25,
    ("NETWORK", "DISK"): 0.2,
    ("APPLICATION", "NETWORK"): 0.15,
    ("SECURITY", "APPLICATION"): 0.1,
    ("CPU", "SECURITY"): 0.1,
...
# Flag to enable/disable transition priors in state encoding
USE_TRANSITION_PRIORS = True

# Compute state dimension based on whether transition priors are enabled
STATE_DIM = 21 if USE_TRANSITION_PRIORS else 20
...
class PPONetwork:
    ...
    Uses STATE_DIM (21 with transition priors, 20 without).
    """
    ...
    def __init__(
        self,
        state_dim:   int = STATE_DIM,   # must match encode_state() output length
        encoder_dim: int = 64,
        lstm_dim:    int = 64,
        seed:        int = 0,
    ) -> None:
...
def encode_state(obs) -> np.ndarray:
    ...
    Layout (20 features when USE_TRANSITION_PRIORS=False, 21 when True):
        ...
        [20]    transition_prior_score (max transition prior for this alert type)
              -- only present when USE_TRANSITION_PRIORS=True

    Total: 20 or 21 features depending on USE_TRANSITION_PRIORS.
    """
    state_dim = 21 if USE_TRANSITION_PRIORS else 20
    if not obs.alerts:
    ...
    # --- Transition prior feature ---
    # Compute max transition probability FROM this alert type to any other type
    # This helps the agent understand cascade risk
    prior = 0.0
    if USE_TRANSITION_PRIORS:
        priors = [TRANSITION_PRIORS.get((primary.alert_type, t), 0.0) 
                  for t in _ALERT_TYPE_MAP.keys()]
        prior = max(priors) if priors else 0.0
    ...
    if USE_TRANSITION_PRIORS:
        feat.append(prior)  # 20 (index) -> 21st feature
        assert len(feat) == 21, f"encode_state returned {len(feat)} features, expected 21"
    else:
...
class PPOTrainer:
    ...
    def __init__(
        ...
        # Network: state_dim must match encode_state() output (21 with transition priors)
        self.net = PPONetwork(state_dim=STATE_DIM, seed=seed)
